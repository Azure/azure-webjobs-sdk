    ...
    internal static class CloudBlobClientExtensions
    {
            ...
            IWebJobsExceptionHandler exceptionHandler, ILogger<BlobListener> logger, CancellationToken cancellationToken)
        {
            ...
            do
            {
                ...
                result = await TimeoutHandler.ExecuteWithTimeout(operationName, context.ClientRequestID, exceptionHandler, logger, t =>
                {
                    return client.ListBlobsSegmentedAsync(prefix, useFlatBlobListing, blobListingDetails, maxResults: null,
                        currentToken: continuationToken, options: null, operationContext: context, cancellationToken: t);
                });
