...
namespace Microsoft.Azure.WebJobs.ServiceBus
{
    internal sealed class EventHubListener : IListener, IEventProcessorFactory
    {
        ...
        {
            public virtual async Task CheckpointAsync(PartitionContext context)
            {
        ...
        internal class EventProcessor : IEventProcessor, IDisposable
        {
            ...
            public EventProcessor(EventHubConfiguration config, Checkpointer checkpointer, ITriggeredFunctionExecutor executor, bool singleDispatch)
            {
            ...
            public async Task ProcessEventsAsync(PartitionContext context, IEnumerable<EventData> messages)
            {
                ...
                if (_singleDispatch)
                {
                    ...
                    for (int i = 0; i < eventCount; i++)
                    {
                        ...
                        var invocationTask = _executor.TryExecuteAsync(input, _cts.Token);
                        invocationTasks.Add(invocationTask);
                    }
                ...
                // Dispose all messages to help with memory pressure.
                // If this is missed, the finalizer thread will still get them. 
