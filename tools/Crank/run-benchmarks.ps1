...
if ($WriteResultsToDatabase) {
    ...
    & $InvokeCrankCommand --config $crankConfigPath --scenario $Scenario --profile local --variable CrankAgentVm=$CrankAgentVm --variable BranchOrCommit=$BranchOrCommit --sql $sqlConnectionString --table FunctionsPerf
} else {
    & $InvokeCrankCommand --config $crankConfigPath --scenario $Scenario --profile local --variable CrankAgentVm=$CrankAgentVm --variable BranchOrCommit=$BranchOrCommit
}
